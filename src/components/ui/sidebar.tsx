...
function SidebarProvider({
  ...
  const [width, setWidthState] = React.useState<number>(() => getSavedSidebarWidth())
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  const handlePointerDown = (event: React.PointerEvent<HTMLButtonElement>) => {
    ...
    const onPointerMove = (moveEvent: PointerEvent) => {
      ...
      if (!isDraggingRef.current && Math.abs(deltaX) > 2) {
  ...
  return (
    <button
      ...
      title="Tarik untuk resize lebar sidebar, klik ganda untuk reset lebar default, klik untuk sembunyikan/tampilkan"
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 transition-all ease-linear group-data-[side=left]:-right-2 group-data-[side=right]:-left-2 sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2",
        "cursor-col-resize hover:w-4",
        "after:absolute after:inset-y-0 after:start-1/2 after:w-[2px] after:transition-colors hover:after:bg-primary/70 after:bg-transparent",
